...
const OWNED_CHANNEL_POLICY =
  ...
  "- 원재료·성분·유지방 등을 근거로 경쟁 제품(예: 하겐다즈)과 비교하며 품질을 깎아내리는 것도 부정입니다(예: '원재료 보소 하겐다즈랑 어딜 비교하노', '원재료 봐라 비교대상이 아니다', '유지방 5%면 프리미엄 대비 매우 낮은 편').\n" +
  "- 댓글·여론 조작 의혹 제기도 부정입니다(예: '댓글알바 작작 써라', '댓글알바 수준', '앵무새라 믿고 거른다', '근거도 없이 엘리트가 만들었다며').\n" +
  "- 영상 내용·주장을 통째로 부정·조롱하는 관용표현도 부정입니다(예: '뭔 헛소리야', '개쌉소리', '가지가지 하네', '족구하고 있네', '지랄이 풍년이네', '듣보 아이스크림').\n" +
  "- 이 지면은 브랜드가 올린 영상·광고이므로, 특정 댓글러나 광고 모델 개인을 지목하지 않은 채 남긴 적대·거부·조롱 한두 마디는 이 영상·광고·브랜드를 향한 것으로 보고 부정입니다(예: '차단', '꺼져', 'ㄲㅈ', '노잼', '극혐', '수준 보소', '뭔 건물을 사 아직 그 정도는 아니여').\n" +
  "- 광고 그 자체에 대한 피로·거부·냉소도 부정입니다(예: '또 광고냐', '광고 지겹다', '이 채널 광고채널이네', '왜 자꾸 뜨나 했더니 광고', '광고라서 거른다').\n" +
  "- 광고 모델·인플루언서 개인의 외모·호감을 긍정적으로 표현하면 정상입니다(예: '○○님 잘생겼다'). 단 연기·연출에 대한 비판·조롱('발연기', '연출 오글거려' 등)은 부정입니다. 단순 '광고네' 인지나 '광고 참신하다/잘 만들었다' 같은 칭찬은 정상입니다.\n" +
  "- 단순 질문·사실 확인·가격/구매처·재고 언급·제품과 무관한 농담은 기존처럼 정상입니다.\n" +
  "- [소유채널] 표시가 없는 일반 협찬·제3자 채널에는 이 확대 정책을 절대 적용하지 마세요.\n";

const AWARENESS_AD_POLICY =
  "\n유료 인지광고 정책([인지광고] 표시가 있는 댓글에만 적용):\n" +
  "- 이 지면은 라라스윗이 돈을 낸 광고입니다. 위 '정상' 예외보다 이 정책이 우선합니다.\n" +
  "- 제품·브랜드와 무관한 타 인물·연예인·지인 언급·비교·드립으로 광고 흐름을 흐리면 부정 (예: '△△ 닮았다', '잠뜰은 박슬기').\n" +
  "- 특정 댓글러나 모델을 지목하지 않은 채 남긴 문맥 없는 적대·거부·조롱 한두 마디도 부정 (예: '차단', '꺼져', 'ㄲㅈ', '노잼', '극혐').\n" +
  "- 광고 모델·연출·연기에 대한 비판·조롱은 부정 (예: '발연기', '연출 오글거려'). 단 모델 개인의 외모·호감 칭찬('○○님 잘생겼다')은 정상.\n" +
  "- 댓글러 간 다툼·시비·신고 유도는 부정 (예: '너나 잘하세요', '신고함').\n" +
  "- 단순 질문·사실 확인·가격/구매처·재고 언급, 팬의 호감·구매의향은 광고에서도 정상.\n";

const PROMPT_HEAD =
  "당신은 '라라스윗'(저당 아이스크림·디저트 브랜드, 대표 제품 '쫀득바') 협찬 게시물의 댓글 검토 담당입니다.\n" +
  "아래 댓글 중 **제품·음식·브랜드를 직접 겨냥한 부정적 언급**만 골라내세요(관리·삭제 대상). 확신이 서지 않으면 정상으로 두세요.\n\n" +
  "부정으로 판단(alert=true):\n" +
  "- 맛·식감·품질·양·가격에 대한 명확한 불만·혹평 (예: '맛없어', '별로', '돈아깝', '과일 저건 좀 에바')\n" +
  "- 구매 만류 (예: '사지 마세요')\n" +
  "- 허위·과대광고 지적, 성분·진위를 정면으로 부정 (예: '허위광고하지마라', '성분표에 멜론이 없던데요?')\n" +
  "- 경쟁 제품을 들어 **라라스윗/쫀득바를 명확히 깎아내리거나 제품의 차별성·우위를 비꼼** (예: '이거 살 바엔 메로나 먹지', '뭔 메로나를 제쳐', '쫀득바보다 메론바가 낫다')\n" +
  "- **광고/바이럴/협찬 자체에 대한 냉소·의심·목적 비꼼, 광고 노출 피로·거부** (예: '또 광고냐', '광고 지겹다', '저거 몇번이나 보는거여', '슈기 홍보임 아스크림 홍보임 ㅋㅋ', '맨날 광고에서만 봤지', '이 채널 광고채널이네')\n" +
  "- **제품·브랜드·회사를 겨냥한** 욕설·비속어·적대·혐오 (예: '이 회사 씨발', '라라스윗 극혐', '라라스윗 왤케 비호감')\n" +
  "- 정치적 비하·조롱·드립 (예: '2찍', 정치인 이름을 건 조롱)\n" +
  "정상으로 판단(alert=false):\n" +
  "- 긍정·중립·감탄·질문·태그·이모지, 제품·브랜드에 대한 호감·구매의향, 이 게시물의 인플루언서·모델에 대한 응원·칭찬.\n" +
  "- **'광고'라는 단어가 있어도 팬이 호감·기대·구매의향을 표현하면 정상.** 예: '후님이 광고하니까 꼭 먹어볼게요🥰 잘생겼어용', '헐 후님이 광고를 하시다니!!', '터후님 광고 너무 잘찍으세요 아자스'.\n" +
  "- **타 인물·연예인·지인 언급, 닮은꼴·비교, 사적인 잡담·드립은 정상** (예: '△△ 닮았다', '재력보소 이재용 저리가라', '우리 이모가 좋아함'). 제품·브랜드를 깎아내리지 않으면 부정 아님. (단 정치적 비하·조롱은 위 부정 항목대로 부정.)\n" +
  "- **댓글러끼리의 캐주얼한 욕설·강조어·티키타카는 정상** (예: '개웃기네', '존나 놀라움', 'ㅈㄴ 웃김', '개처먹엇어요'). 욕설·강조어가 제품·브랜드·회사를 겨냥하지 않았으면 부정 아님.\n" +
  "- 댓글러 간 가벼운 시비·농담·신고 드립도 제품·브랜드 비방이 아니면 정상 (예: '왜이렇게 화가 났어요ㅋㅋ', '너나 잘하세요').\n" +
  "- **경쟁 제품을 단순 언급·사실 정정·취향 표현만** 하고 라라스윗/쫀득바를 깎아내리지 않으면 정상 (예: '메로나도 맛있죠', '걍 메로나임', '메론바가 원조지'). 경쟁품 이름이 있다고 부정 아님.\n" +
  "- 모호한 아쉬움·잡담, 외국어로 된 잡담은 명확한 제품·브랜드 비방이 아니면 정상 (예: '뭔가 좀 부족한 느낌', '양 줄었나?').\n" +
  "- 다른 매장·다른 제품·재고 등 **제품/브랜드를 직접 깎아내리지 않는 가용성·사실 관찰은 정상.** 예: '지금 베라 갈려고 했는데 이게 왜 나오는데', 'gs네 cu면 갈만했는데 까비'.\n" +
...
function buildPrompt(chunk) {
  // 인지광고 마커가 소유채널 마커보다 우선한다(유튜브 광고는 두 플래그를 모두 가질 수 있다).
  const hasAwarenessContext = chunk.some((comment) => comment?.awarenessAdScope === true);
  const hasOwnedChannelContext = chunk.some((comment) => (
    comment?.ownedChannelBrandHostilityScope === true && comment?.awarenessAdScope !== true
  ));
  const numbered = chunk.map((comment, index) => {
    const scope = comment?.awarenessAdScope === true ? '[인지광고] '
      : comment?.ownedChannelBrandHostilityScope === true ? '[소유채널] '
        : '';
