...
test('단순 광고·바이럴 의심은 LLM 검토 대상으로 남기되 키워드 폴백에서는 통과', () => {
  assert.equal(classifyNegativeComment({ text: '또 바이럴이네' }, T).alert, false);
  assert.equal(needsContextualReview({ text: '또 바이럴이네' }, T), true);
});
